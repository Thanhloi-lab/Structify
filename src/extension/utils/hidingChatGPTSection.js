...
(function () {
  ...
  function isMasterSettingEnabled() {
    return getSetting('allowChatGPTOptimize', true)
  }

  async function start() {
    if (!isMasterSettingEnabled()) return false;
    if (isActive) return true;

    const targetSelector = getSetting('chatContainerSelector', '#main div.flex.flex-col.text-sm');
    const container = document.querySelector(targetSelector);
  ...
  async function getStats() {
    ...
    if (autoStart && !isActive) {
      globalThis.HidingChatGPTSection.start();
    }
  ...
  async function shouldAutoStart() {
    ...
    return isMasterSettingEnabled() === true && settings.isRunning === true;
  }

  globalThis.HidingChatGPTSection = {
    start,
    stop,
    manualRestore,
    toggle,
